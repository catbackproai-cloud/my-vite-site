...
function App() {
  ...
  var NS="cat_back";
  var I="https://bot.heroai.pro/?ns="+encodeURIComponent(NS)+"&hideLauncher=1";
  var CACHE_KEY="hero_ns_cfg_"+NS;

  /* -------- critical CSS first: no flash -------- */
  var css = ""
  + ".heroai-launcher{position:fixed;right:20px;bottom:20px;min-width:56px;height:56px;padding:0 18px;border:0;border-radius:999px;font-weight:700;box-shadow:0 12px 28px rgba(13,27,62,.15);cursor:pointer;z-index:2147483646;display:inline-flex;align-items:center;justify-content:center;visibility:hidden}"
  ...
  /* -------- pull cached cfg immediately (prevents default blue/label flash) -------- */
  var cached=null;
  try{ cached=JSON.parse(localStorage.getItem(CACHE_KEY)||"null"); }catch(_){}
  var ui0 = cached && cached.ui || {};
  var c0  = cached && cached.colors || {};

  var label0 = (ui0.launcher_text || "Chat").trim();
  var bg0    = (c0.launcher_bg || "#0a4fd3").trim();
  var fg0    = (c0.launcher_text || "#ffffff").trim();

  /* -------- build DOM with cached styles -------- */
  var wrap=document.createElement("div"); wrap.className="heroai-iframe-wrap";
  var iframe=document.createElement("iframe");
  iframe.className="heroai-iframe"; iframe.src=I; iframe.allow="clipboard-read; clipboard-write"; iframe.title="Hero AI Assistant";
  wrap.appendChild(iframe); document.body.appendChild(wrap);

  var btn=document.createElement("button"); btn.className="heroai-launcher";
  btn.textContent=label0; btn.style.background=bg0; btn.style.color=fg0;
  document.body.appendChild(btn);
  // show only after styled
  btn.style.visibility="visible";

  /* -------- open/close -------- */
  var open=false;
  function postOpen(){ try{ iframe.contentWindow.postMessage({type:"HERO_CHAT_OPEN"},"*"); }catch(e){} }
  function setOpen(v){ open=v; wrap.style.display=v?"block":"none"; if(v) postOpen(); }
  btn.addEventListener("click", function(){ setOpen(!open); });

  /* optional: if iframe signals ready and we're already open, re-send */
  window.addEventListener("message", function(e){
    if (e.source!==iframe.contentWindow) return;
    if (e.data && e.data.type==="HERO_READY" && open) postOpen();
  });

  /* -------- ns-config fetch: update + recache + auto-open -------- */
  function scheduleAutoOpen(enabled, delayMs, onceMode){
    if(!enabled) return;
    var sk="hero_auto_opened_"+NS, dk="hero_last_open_"+NS, now=Date.now();
    if(onceMode==="session" && sessionStorage.getItem(sk)==="1") return;
    if(onceMode==="day"){ var last=parseInt(localStorage.getItem(dk)||"0",10); if(now-last<86400000) return; }
    var start=function(){ if(open) return; setTimeout(function(){ if(open) return; setOpen(true);
      if(onceMode==="session") sessionStorage.setItem(sk,"1");
      if(onceMode==="day") localStorage.setItem(dk,String(Date.now()));
    }, delayMs); };
    if(document.visibilityState==="visible") start();
    else { var onVis=function(){ if(document.visibilityState==="visible"){ document.removeEventListener("visibilitychange",onVis); start(); } };
  ...
  fetch("https://n8n.srv845865.hstgr.cloud/webhook/ns-config?ns="+encodeURIComponent(NS)+"&_="+Date.now(),
    ...
    .then(function(cfg){
      ...
      var ui=cfg.ui||{}, c=cfg.colors||{};
      // apply fresh styles/label (won’t flash; just updates if changed)
      btn.textContent=(ui.launcher_text||label0).trim();
      ...
      // auto-open (defaults: enabled, 3000ms, once per session)
      var enabled = ui.autostart!==false;
      var delayMs = Number(ui.autostart_ms||3000);
      var onceMode= "session";
      scheduleAutoOpen(enabled, delayMs, onceMode);
    })
    .catch(function(){ /* keep cached values; no flash */ });
