...
function App() {
  ...
  // --- critical CSS (injected before DOM changes, no FOUC) ---
  var css = ".heroai-launcher{position:fixed;right:20px;bottom:20px;min-width:56px;height:56px;padding:0 18px;border:0;border-radius:999px;font-weight:700;box-shadow:0 12px 28px rgba(13,27,62,.15);cursor:pointer;z-index:2147483646;display:inline-flex;align-items:center;justify-content:center;background:#0a4fd3;color:#fff}"
  ...
  btn.className="heroai-launcher";btn.textContent="Chat";
  document.body.appendChild(btn);

  // --- open/close ---
  var open=false;
  function setOpen(v){open=v;wrap.style.display=v?"block":"none";if(v)try{iframe.contentWindow.postMessage({type:"HERO_CHAT_OPEN"},"*");}catch(e){}}
  btn.onclick=function(){setOpen(!open)};

  // --- ns-config fetch (label/colors/autostart) ---
  fetch("https://n8n.srv845865.hstgr.cloud/webhook/ns-config?ns="+encodeURIComponent(NS)+"&_="+Date.now(),{headers:{"cache-control":"no-cache"}})
    .then(r=>r.json()).then(cfg=>{
      var ui=cfg.ui||{},c=cfg.colors||{};
      btn.textContent=(ui.launcher_text||"Chat").trim();
      if(c.launcher_bg)btn.style.background=c.launcher_bg.trim();
      if(c.launcher_text)btn.style.color=c.launcher_text.trim();

      // auto-open
      var enabled=ui.autostart!==false, delay=Number(ui.autostart_ms||3000);
      if(enabled && !sessionStorage.getItem("hero_opened_"+NS)){
        setTimeout(()=>{setOpen(true);sessionStorage.setItem("hero_opened_"+NS,"1");},delay);
      }
    })
    .catch(()=>{});
